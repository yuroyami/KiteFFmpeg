...
class PipelineRoundTripTest {
    ...
    private fun tmp(name: String): String {
        ...
        return "$root/kiteffmpeg-test-$name".also { tmpFiles += it }
    }
    ...
    fun streamMetadataIsExposed() {
        ...
        writeTestVideo(src)
        runBlocking { Remuxer.remux(src, dst, metadata = mapOf("title" to "kiteffmpeg test")) }
        MediaSource.open(dst).use { s ->
            assertEquals("kiteffmpeg test", s.metadata["title"] ?: s.metadata["TITLE"])
