...
> **GPL build variant of KiteFFmpeg.** Drop-in replacement for `kiteffmpeg` that adds libx264 / libx265 for quality-focused software H.264 / H.265 encode.
...
For those cases, use **`kiteffmpeg`** (LGPL) instead, paired with platform hardware encoders (`h264_videotoolbox`, `h264_mediacodec`; NVENC and WebCodecs are on the roadmap) or `libsvtav1` for AV1.

## Status

**Skeleton module, not yet implemented.** It is not built or published, and it is commented out of `settings.gradle.kts`.

The FFmpeg build side already exists in `buildSrc`: `BuildFFmpegTask` builds **LGPL by default**, and the GPL flavour (`--enable-gpl --enable-version3 --enable-libx264 --enable-libx265`) is available today through the `:kiteffmpeg:buildFFmpegFor<Target>Gpl` task variants, which write to `native-libs/gpl/<target>/`. Building `kiteffmpeg` with `-Pkiteffmpeg.ffmpeg.license=gpl` links against that tree. What remains is packaging that flavour as this drop-in artifact.
...
implementation("io.github.yuroyami:kiteffmpeg:0.x.x")
