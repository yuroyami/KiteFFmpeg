...
class JniIdentityTest {
    ...
    fun identityHasAllTypedFieldsAndTheCurrentAbi() {
        // The pin is deliberate: an ABI bump nobody wrote down must fail here. 2.9 adds the
        // descriptions and commentary disposition constants; move this with KITECODEC_C_ABI_MINOR only.
        val identity = FFmpeg.identity
        assertTrue(identity.isAcceptable, identity.describe())
        assertEquals("2.9", identity.cAbiVersion)
        assertEquals(
