...
class JniIdentityTest {
    ...
    fun identityHasAllTypedFieldsAndTheCurrentAbi() {
        // The pin is deliberate: an ABI bump nobody wrote down must fail here. 3.5 adds subtitle
        // decoding; move this with KITECODEC_C_ABI_MAJOR and _MINOR only.
        val identity = FFmpeg.identity
        assertTrue(identity.isAcceptable, identity.describe())
        assertEquals("3.5", identity.cAbiVersion)
        assertEquals(
