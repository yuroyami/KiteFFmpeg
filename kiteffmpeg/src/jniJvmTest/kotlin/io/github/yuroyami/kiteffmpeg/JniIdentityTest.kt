...
class JniIdentityTest {
    ...
    fun identityHasAllTypedFieldsAndTheCurrentAbi() {
        // The pin is deliberate: an ABI bump nobody wrote down must fail here. 3.3 adds the
        // stream identity copy and the chapter write; move this with KITECODEC_C_ABI_MAJOR and _MINOR only.
        val identity = FFmpeg.identity
        assertTrue(identity.isAcceptable, identity.describe())
        assertEquals("3.3", identity.cAbiVersion)
        assertEquals(
