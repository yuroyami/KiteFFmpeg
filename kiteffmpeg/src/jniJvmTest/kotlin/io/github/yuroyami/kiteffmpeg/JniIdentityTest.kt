...
class JniIdentityTest {
    ...
    fun identityHasAllTypedFieldsAndTheCurrentAbi() {
        // The pin is deliberate: an ABI bump nobody wrote down must fail here. 3.6 adds the custom
        // output bridge; move this with KITECODEC_C_ABI_MAJOR and _MINOR only.
        val identity = FFmpeg.identity
        assertTrue(identity.isAcceptable, identity.describe())
        assertEquals("3.6", identity.cAbiVersion)
        assertEquals(
