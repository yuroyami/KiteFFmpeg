...
/**
 * Runs a transcode's blocking [work] on [dispatcher], and hands every progress report the work
 * publishes to [onProgress] in the caller's own coroutine context.
 *
 ...
 */
internal suspend fun runTranscode(
    dispatcher: CoroutineDispatcher,
    onProgress: ((TranscodeProgress) -> Unit)?,
    work: suspend (publish: ((TranscodeProgress) -> Unit)?) -> Unit,
) {
    if (onProgress == null) {
    ...
    coroutineScope {
        val reports = Channel<TranscodeProgress>(Channel.CONFLATED)
        launch(dispatcher) {
            try {
