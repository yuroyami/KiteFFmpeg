 ...
 */
class CompileKiteFFmpegCTaskTest {
    ...
    fun aCorrectObjectArchives() {
        ...
        val archive = fixture.outputRoot.resolve("macos_arm64/${CompileKiteFFmpegCTask.ARCHIVE_NAME}")
        ...
        assertContains(describe(objectFile), CompileKiteFFmpegCTask.expectedObjectDescription("macos_arm64"))
    }
    ...
    fun anObjectOfTheWrongArchitectureIsRefusedNamingBothArchitecturesAndTheTarget() {
        ...
        val failure = assertFails {
            CompileKiteFFmpegCTask.verifyObjectArchitecture("macos_arm64", x64Object, actual)
        }
        val message = failure.message ?: ""
        // Both architectures, so the reader does not have to guess which end is wrong.
        assertContains(message, actual)
        assertContains(message, CompileKiteFFmpegCTask.expectedObjectDescription("macos_arm64"))
    ...
    ) : CompileKiteFFmpegCTask(execOperations) {
    ...
    fun theLlvmPackageResolverPrefersTheNamedPackage() {
        ...
        val resolved = CompileKiteFFmpegCTask.resolveLlvmBinDir(root, "llvm-21-aarch64-macos-essentials-97")
    ...
    fun theLlvmPackageResolverFallsBackNumericallyAndSaysSo() {
        ...
        val resolved = CompileKiteFFmpegCTask.resolveLlvmBinDir(root, "llvm-22-does-not-exist") { messages += it }
    ...
    fun noLlvmPackageAtAllFailsNamingWhatIsMissing() {
        val root = createTempDirectory()
        val failure = assertFails {
            CompileKiteFFmpegCTask.resolveLlvmBinDir(root, "llvm-21-aarch64-macos-essentials-97")
        }
    ...
    fun aWindowsShapedDependenciesTreeResolvesTheExeNames() {
        ...
        val resolved = CompileKiteFFmpegCTask.resolveLlvmBinDir(root, "llvm-21-x86_64-windows-essentials-97")
        assertEquals(bin.absolutePath, resolved.absolutePath)
        assertEquals("clang.exe", CompileKiteFFmpegCTask.resolveTool(resolved, "clang")?.name)
        assertEquals("llvm-ar.exe", CompileKiteFFmpegCTask.resolveTool(resolved, "llvm-ar")?.name)
    }
    ...
    fun theAndroidToolchainPackageIsNamedAfterTheBuildHost() {
        ...
        assertEquals("osx", CompileKiteFFmpegCTask.konanHostInfix("Mac OS X"))
        assertEquals("windows", CompileKiteFFmpegCTask.konanHostInfix("Windows Server 2022"))
        assertEquals("linux", CompileKiteFFmpegCTask.konanHostInfix("Linux"))
        assertTrue(
            root.resolve(CompileKiteFFmpegCTask.androidToolchainSysroot("Linux")).isDirectory,
    ...
    private fun newTask(konanTarget: String, fixture: Fixture): CompileKiteFFmpegCTask {
        val project = ProjectBuilder.builder().build()
        val task = project.tasks
            .register("compileKiteFFmpegCFor$konanTarget", CompileKiteFFmpegCTask::class.java)
            .get()
    ...
    fun buildDefinesBecomeQuotedMinusDArgumentsInNameOrder() {
        assertEquals(
            ...
            CompileKiteFFmpegCTask.defineArguments(
                mapOf(
                    CompileKiteFFmpegCTask.DEFINE_FFMPEG_REF to "n8.0",
                    CompileKiteFFmpegCTask.DEFINE_FFMPEG_DIR to "/opt/homebrew/lib",
                    CompileKiteFFmpegCTask.DEFINE_FFMPEG_LICENSE to "lgpl",
                ),
            ),
        )
        assertEquals(emptyList(), CompileKiteFFmpegCTask.defineArguments(emptyMap()))
    }
    ...
    fun ffmpegIncludesAreSearchedAfterTheSysrootAndOnlyOursUsesDashI() {
        val args = CompileKiteFFmpegCTask.includeArguments(
    ...
    fun onlyTheProjectIncludeIsEmittedWhenThereAreNoFfmpegDirectories() {
        assertEquals(
            ...
            CompileKiteFFmpegCTask.includeArguments(ownInclude = "/work/include", ffmpegIncludes = emptyList()),
        )
    }
    ...
    fun bothSpellingsOfAnX8664CoffObjectAreAccepted() {
        ...
        CompileKiteFFmpegCTask.verifyObjectArchitecture(
            "mingw_x64", stand_in,
     ...
     * on Windows is a path like `D:\a\KiteFFmpeg`, whose backslashes are escape sequences inside a
    ...
    fun theGeneratedHeaderEscapesWindowsPathsIntoValidCStringLiterals() {
        val header = CompileKiteFFmpegCTask.buildDefinesHeader(
            mapOf(
                "KC_BUILD_FFMPEG_REF" to "n8.0",
                "KC_BUILD_FFMPEG_DIR" to """D:\a\KiteFFmpeg\native-libs""",
            ),
        )
        assertContains(header, """#define KC_BUILD_FFMPEG_REF "n8.0"""")
        assertContains(header, """#define KC_BUILD_FFMPEG_DIR "D:\\a\\KiteFFmpeg\\native-libs"""")
    }

    /** A value carrying a quote must not be able to end the literal early. */
    @Test
    fun theGeneratedHeaderEscapesEmbeddedQuotes() {
        val header = CompileKiteFFmpegCTask.buildDefinesHeader(mapOf("KC_X" to """a"b"""))
    ...
    fun theGeneratedHeaderIsSortedAndStable() {
        val header = CompileKiteFFmpegCTask.buildDefinesHeader(mapOf("KC_Z" to "z", "KC_A" to "a"))
    ...
    fun `both macOS C targets compile at the one deployment floor`() {
        ...
        assertEquals(
            ...
            CompileKiteFFmpegCTask.specFor("macos_arm64").triple,
        )
        assertEquals(
            "x86_64-apple-macos$floor",
            CompileKiteFFmpegCTask.specFor("macos_x64").triple,
        )
    }
